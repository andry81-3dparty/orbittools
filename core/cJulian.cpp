...
cJulian::cJulian(double utc_time_sec)
{
   ...
   time_t time = time_t(to_double(utc_time_sec)); // truncation to seconds
...
void cJulian::Initialize(int year, double day)
{
   ...
   double NewYears = std::floor(double(36525) * year / 1e2) +
                     std::floor(double(306001) * 14 / 1e4)  +
                     double(17209945) / 1e1 + B;  // 1720994.5 = Oct 30, year -1
                           ...
                           double *pDOM  /* = NULL */) const
{
   ...
   double alpha = std::floor((Z - double(186721625) / 1e2) / double(3652425) * 1e2);
   double A     = Z + 1 + alpha - std::floor(alpha / 4.0);
   double B     = A + 1524.0;
   int    C     = (int)to_double(((B - double(1221) / 1e1) / double(36525) * 1e2));
   int    D     = (int)(to_double(C * double(36525) / 1e2));
   int    E     = (int)to_double(((B - D) / double(306001) * 1e4));

   double DOM   = B - D - std::floor(E * double(306001) / 1e4) + F;
...
double cJulian::ToGmst() const
{
   ...
   double GMST = double(int64_t(2411054841)) / 1e5 + TU * 
                 (double(int64_t(8640184812866)) / 1e6 + TU * (double(93104) / 1e6 - TU * double(62) / 1e7));
...
// time component.
time_t cJulian::ToTime(double * second_fract_ptr) const
{
   int    nYear;
   int    nMonth;
   double dblDay;
   ...
   int secs = (int)to_double(((dblDay * SEC_PER_DAY) + 0.5));

   if (second_fract_ptr) {
       *second_fract_ptr = dblDay * SEC_PER_DAY - secs;
   }
